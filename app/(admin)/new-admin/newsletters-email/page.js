...
export default async function NewAdminNewsletterPage({ searchParams }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <form className="flex max-w-xl gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><label className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-slate-400" /><input name="q" defaultValue={params.q} className={`${inputClass} pl-9`} placeholder="Search subscriber email" /></label><LoadingButton className="h-10 rounded-xl bg-indigo-600 px-5 text-sm font-medium text-white">Search</LoadingButton></form>
