...
/**
 * Resolve Clerk user IDs used in order history
 * into admin name + email.
 */
async function getAdminMap(statusHistory = []) {
  const userIds = [
    ...new Set(
      statusHistory
  ...
  const results = await Promise.all(
    userIds.map(async (userId) => {
      try {
        ...
        console.error(
          `Failed to load Clerk admin ${userId}:`,
          error
        );
...
export default async function NewAdminOrderDetailPage({ params }) {
  ...
  /**
   * Resolve all admins who changed this order.
   *
   * Example:
   * user_123456 -> Praveen Kumar / admin@email.com
   */
  const adminMap = await getAdminMap(order.statusHistory);
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,.65fr)]">
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <a
                ...
                className="mt-1 block text-indigo-600"
              >
                {order.shipPhone}
              </a>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      SKU {item.productSku} · {item.color} · Size{" "}
                      {item.size}
                    </p>
            ...
            <div className="space-y-2 border-t border-slate-100 bg-slate-50 p-5 text-sm">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-medium">
                  {formatPrice(payment?.amount ?? order.total)}
                </dd>
              </div>
            ...
            {order.isCod ? (
              <p
                className={`mt-4 rounded-xl p-3 text-xs ${order.status === "DELIVERED"
                    ? "bg-emerald-50 text-emerald-700"
                    : "bg-amber-50 text-amber-800"
                  }`}
              >
                {order.status === "DELIVERED"
                  ? "COD revenue is realized."
                  : "COD remains outstanding until delivery."}
              </p>
            ) : null}
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              >
                {order.trackingStatus || "Not added"}
              </StatusBadge>
            </div>
            ...
            <dl className="mt-4 space-y-3 text-sm">
              <div>
                ...
                <dd className="font-medium">India Post</dd>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <ol className="mt-5 space-y-0">
              ...
              <li className="flex gap-3">
                <div className="flex flex-col items-center">
                  <span className="mt-0.5 size-3 rounded-full bg-indigo-600" />

                  <span className="h-14 w-px bg-indigo-200" />
                </div>
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        {index <
                          ...
                          <span className="min-h-16 w-px bg-indigo-200" />
                        ) : null}
                      </div>
            ...
            {!order.statusHistory?.length ? (
              <p className="mt-3 text-xs text-slate-500">
                No recorded transitions yet. Existing orders
                begin recording history with their next
                change.
              </p>
            ) : null}
          </section>
