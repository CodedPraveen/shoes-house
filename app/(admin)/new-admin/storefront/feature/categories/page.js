...
import {
  CreateCategoryForm,
  EditCategoryForm,
} from "@/components/new-admin/categories/category-form";
import {
  EmptyState,
  PageHeader,
  StatusBadge,
...
export default async function CategoryManagementPage({
  ...
  const collection =
    params.collection === "JEWELLERY"
      ? "JEWELLERY"
      : "SHOES";

  const categories =
    await categoryAdminService.list(collection);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        title="Categories"
        description="Manage categories and their storefront images. Category slugs and product relationships stay unchanged when names are edited."
        action={
          <Link
      ...
      <div className="flex gap-2">
        <Link
          ...
          className={`rounded-xl px-4 py-2 text-sm ${collection === "SHOES"
        ...
        <Link
          ...
          className={`rounded-xl px-4 py-2 text-sm ${collection === "JEWELLERY"
              ? "bg-slate-950 text-white"
              : "border border-slate-200 bg-white"
            }`}
        >
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <h2 className="text-lg font-semibold">
          Add {collectionLabel} category
        </h2>

        <p className="mt-1 text-sm text-slate-500">
          Add a category and optionally upload its storefront image.
        </p>

        <div className="mt-5">
          <CreateCategoryForm
            key={`${collection}-${categories.length}`}
        ...
        {categories.length ? (
          ...
          <div className="mt-5">
            <EmptyState
              ...
              description={`Add the first ${collectionLabel.toLowerCase()} category above.`}
            />
          </div>
        )}
      </section>
