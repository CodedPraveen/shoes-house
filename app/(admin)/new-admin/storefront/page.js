...
function TargetFields({ categories, products, item }) {
  return (
    <>
      <select name="targetType" defaultValue={item?.targetType || "COLLECTION"} className={inputClass}>
        <option value="COLLECTION">Collection home</option>
        <option value="CATEGORY">Category</option>
        <option value="PRODUCT">Product</option>
        <option value="CUSTOM">Custom local path</option>
      </select>
      <select name="categoryId" defaultValue={item?.categoryId || ""} className={inputClass}><option value="">No category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select>
      <select name="productId" defaultValue={item?.productId || ""} className={inputClass}><option value="">No product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select>
      <input name="customHref" defaultValue={item?.customHref || ""} className={inputClass} placeholder="/shoes/products" />
    </>
  );
}
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          {data.productSections.map((section, index) => (
            <div key={section.key} className="rounded-xl border border-slate-200 p-4">
              <form action={saveStorefrontSectionAction} className="grid gap-3 sm:grid-cols-2">
                ...
                <button className={`${buttonClass} sm:col-span-2`}>Save section</button>
              </form>

              <div className="mt-3 flex gap-2">
                <form action={moveStorefrontProductSectionAction}><HiddenCollection collection={collection} /><input type="hidden" name="key" value={section.key} /><input type="hidden" name="direction" value="up" /><button disabled={index === 0} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-40">Move up</button></form>
                <form action={moveStorefrontProductSectionAction}><HiddenCollection collection={collection} /><input type="hidden" name="key" value={section.key} /><input type="hidden" name="direction" value="down" /><button disabled={index === data.productSections.length - 1} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-40">Move down</button></form>
              </div>

              <div className="mt-4 border-t border-slate-100 pt-4">
                <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-400">Selected products</p>
                <div className="space-y-2">{section.items?.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-sm"><span className="min-w-0 flex-1 truncate">{item.product?.name || "Unavailable product"}</span><form action={reorderStorefrontSectionItemAction} className="flex items-center gap-2"><HiddenCollection collection={collection} /><input type="hidden" name="id" value={item.id} /><input type="number" name="sortOrder" defaultValue={item.sortOrder} className="h-8 w-16 rounded-lg border border-slate-200 px-2 text-xs" aria-label="Product order" /><button className="text-xs font-medium text-indigo-600">Save order</button></form><form action={disableStorefrontSectionItemAction}><HiddenCollection collection={collection} /><input type="hidden" name="id" value={item.id} /><button className="text-xs font-medium text-rose-600">Remove</button></form></div>)}</div>
                <form action={saveStorefrontSectionItemAction} className="mt-3 grid gap-2 sm:grid-cols-2">
                  ...
                  <button className={`${buttonClass} sm:col-span-2`}>Add product</button>
                </form>
              </div>
            </div>
          ))}
          <form action={createStorefrontProductSectionAction} className="grid gap-3 rounded-xl border border-dashed border-indigo-300 bg-indigo-50/40 p-4 sm:grid-cols-2">
            ...
            <button className={buttonClass}>Add section</button>
          </form>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          {data.navbarItems.map((item) => <form key={item.id} action={saveNavbarItemAction} className="grid gap-2 rounded-xl border border-slate-200 p-4 sm:grid-cols-2"><HiddenCollection collection={collection} /><input type="hidden" name="id" value={item.id} /><input name="label" defaultValue={item.label} className={inputClass} required /><input type="number" name="sortOrder" defaultValue={item.sortOrder} className={inputClass} /><TargetFields categories={data.categories} products={data.products} item={item} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="enabled" defaultChecked={item.enabled} />Enabled</label><button className={buttonClass}>Save item</button></form>)}
          <form action={saveNavbarItemAction} className="grid gap-2 rounded-xl border border-dashed border-indigo-300 bg-indigo-50/40 p-4 sm:grid-cols-2"><HiddenCollection collection={collection} /><p className="font-semibold sm:col-span-2">Add navigation item</p><input name="label" className={inputClass} placeholder="Label" required /><input type="number" name="sortOrder" defaultValue={data.navbarItems.length} className={inputClass} /><TargetFields categories={data.categories} products={data.products} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="enabled" defaultChecked />Enabled</label><button className={buttonClass}>Add item</button></form>
        </div>
      </section>
    </div>
  );
