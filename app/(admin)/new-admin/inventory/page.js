...
export default async function NewAdminInventoryPage({ searchParams }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <form className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-[2fr_1fr_auto]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-slate-400" /><input name="q" defaultValue={params.q} className={`${inputClass} pl-9`} placeholder="Product name or SKU" /></label><select name="stock" defaultValue={params.stock ?? ""} className={inputClass}><option value="">All stock</option><option value="low">Low stock</option><option value="out">Out of stock</option><option value="in">In stock</option></select><LoadingButton className="h-10 rounded-xl bg-indigo-600 px-5 text-sm font-medium text-white">Apply</LoadingButton></form>
