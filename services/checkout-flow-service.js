...
import { addressService, toCheckoutAddress } from "@/services/address-service";
import { assertRateLimit } from "@/lib/rate-limit";
import { requireDbUser } from "@/lib/require-db-user";
import { firstAddressError, validateAddressInput } from "@/lib/address-validation";
...
async function resolveShippingAddress(userId, { addressId, ...manual }, options) {
  ...
  if (!result.isValid) throw new Error(firstAddressError(result.errors));
  return result.address;
}

export async function createBuyNowCheckoutSession(input) {
  const user = await requireDbUser();

  try {
    await assertRateLimit({ prefix: "checkout-buy-now", limit: 8, windowMs: 60_000 });
    const shipping = await resolveShippingAddress(user.id, input, { requireEmail: true });
    ...
    if (!productId || !size) return { ok: false, error: "Missing product selection" };
  ...
  try {
    ...
    if (payment.status !== "captured" && payment.captured !== true) {
      return {
        ok: false,
        pending: true,
        error: "Payment was received and is still being confirmed. Please check My Orders shortly.",
      };
    }
    ...
    return {
      ...
      paid: true,
      recoverable: true,
      error: "Payment was received, but we could not finish saving the order yet. Please check My Orders shortly; do not pay again.",
    };
  }
}
