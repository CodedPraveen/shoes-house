...
const LOCAL_PRODUCT_IMAGE_PATTERN =
  /^\/images\/products\/[A-Za-z0-9_-]{1,100}\/i_[a-f0-9]{32}\.webp$/;

const LOCAL_STORAGE_PATH_PATTERN =
  /^(products|categories|banners)\/[A-Za-z0-9_-]{1,100}\/i_[a-f0-9]{32}\.webp$/;

const STAGING_IMAGE_PATTERN =
  /^\/api\/admin\/images\/staging\/i_[a-f0-9]{32}$/;

function localStorageImageUrl(storagePath) {
  const normalized = String(storagePath || "").replaceAll("\\", "/");

  if (!LOCAL_STORAGE_PATH_PATTERN.test(normalized)) {
    return null;
  }
...
export function isStagingProductImage(value) {
  return (
    typeof value === "string" &&
    STAGING_IMAGE_PATTERN.test(value.trim())
  );
}
...
export function validateImageSource(
  value,
  { allowLocal = false, allowStaging = false } = {},
) {
  if (value == null) {
    return invalid(
  ...
  return invalid(
    ...
    "Externally hosted product images are not supported. Upload the image from this device.",
    parsed.toString(),
  );
}
...
export function validateProductImageUrl(value) {
  if (
    typeof value === "string" &&
    LOCAL_PRODUCT_IMAGE_PATTERN.test(value.trim())
  ) {
...
export function validateProductImages(images) {
  ...
  const validations = list.map((image, index) => {
    const source =
      typeof image === "string"
        ? image
        : localStorageImageUrl(image?.storagePath) ||
        image?.url ||
        "";

    return {
      index,
      ...validateProductImageUrl(source),
    };
  });

  const invalidImages = validations.filter(
    (image) => !image.isValid,
  );
...
export function getProductImageValidation(product) {
  if (product?.imageValidation?.code) {
    return product.imageValidation;
  }
