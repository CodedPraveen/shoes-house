...
export function getWorkflowLabel(order) {
  ...
  if (order.status === "PROCESSING") return order.trackingNumber ? "Sending order" : "Confirmed";
  if (order.status === "PENDING" && !order.confirmedByCall && isCodOrder(order)) {
    return "Confirm by call";
  }
  return "New order";
}
