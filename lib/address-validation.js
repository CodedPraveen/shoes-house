...
export function validateAddressInput(input = {}, { requireEmail = false } = {}) {
  const address = {
    label: normalize(input.label, FIELD_LIMITS.label) || "Home",
    fullName: normalize(input.fullName, FIELD_LIMITS.fullName),
    email: normalize(input.email, FIELD_LIMITS.email).toLowerCase(),
  ...
  if (requireEmail && !/^\S+@\S+\.\S+$/.test(address.email)) {
    errors.email = "Enter a valid email address.";
  }
  if (
    !/^[6-9]\d{9}$/.test(address.phone) ||
    /^(\d)\1{9}$/.test(address.phone)
  ) {
