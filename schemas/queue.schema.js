...
export const categoryImageJobSchema = z.object({
...
export const bullMqEnvironmentSchema = z.object({
  ...
  BULLMQ_WORKER_CONCURRENCY: z.coerce
    .number()
    .int()
    .min(1)
    .max(20)
  ...
  BULLMQ_WORKER_RATE_MAX: z.coerce
    .number()
    .int()
    .min(1)
    .max(1000)
  ...
  BULLMQ_WORKER_RATE_DURATION_MS: z.coerce
    .number()
    .int()
    .min(1000)
    .max(60000)
  ...
  BULLMQ_REDIS_CONNECT_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .min(500)
    .max(30000)
...
export const workerEnvironmentSchema =
  bullMqEnvironmentSchema.extend({
    DATABASE_URL: z.string().trim().min(1),
    IMAGE_STORAGE_ROOT: z.string().trim().min(1).optional(),
  });

export function getBullMqEnvironment(
