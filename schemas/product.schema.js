...
export async function enqueueProductImages(payload) {
  const parsed = productImageJobSchema.parse({
    ...payload,
    type: "product",
  });
...
export async function getProductImageJobState(jobId) {
  const job = await getProductImageQueue().getJob(
    String(jobId || ""),
  );
...
export async function retryProductImages(
  payload,
  existingJobId = null,
) {
  const parsed = productImageJobSchema.parse({
  ...
  const jobId =
    existingJobId || productImageJobId(parsed);
...
export async function enqueueBannerImage(payload) {
  const parsed = bannerImageJobSchema.parse({
    ...payload,
    type: "banner",
  });

  return getProductImageQueue().add(
    "finalize-banner-image",
    parsed,
    {
...
export async function enqueueCategoryImage(payload) {
  const parsed = categoryImageJobSchema.parse({
    ...payload,
    type: "category",
  });

  return getProductImageQueue().add(
    "finalize-category-image",
    parsed,
    {
